/**
 * exp_ChemistryLaTeX_Ind - Chemistry Formula Renderer v7.0 (Independent Client-Side)
 * Works on ChatGPT and other strict CSP sites
 * 
 * CLIENT-SIDE ARCHITECTURE (NO SERVER DEPENDENCY):
 * - SVG rendering via SmilesDrawer (bundled)
 * - Direct API queries to PubChem, RCSB, COD
 * - Extension handles 3D viewers (MolView, Mol*, 3Dmol.js)
 * - MineralNames.js fallback when COD is down
 * - Settings apply instantly
 * 
 * This version is for trusted developers, VIPs, and personal use.
 * For lower latency as no server middleware is used.
 */

// ============================================
// CLIENT-SIDE RENDERING MODE
// ============================================
// This extension uses SmilesDrawer for 2D rendering (no server needed)
// and queries PubChem, RCSB, COD APIs directly.

// Cache version - increment when rendering format changes
const CHEMTEX_CACHE_VERSION = 'v7-client';
...
// ============================================
// These are queried directly without a server middleware
const API_ENDPOINTS = {
  PUBCHEM_NAME_TO_CID: 'https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/',
  PUBCHEM_CID_PROPS: 'https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/',
  ...
  RCSB_STRUCTURE_IMG: 'https://cdn.rcsb.org/images/structures/',
  COD_SEARCH: 'https://www.crystallography.net/cod/result',
  COD_CIF: 'https://www.crystallography.net/cod/',
  OPSIN_IUPAC: 'https://opsin.ch.cam.ac.uk/opsin/'
};

// ============================================
// SMILESDRAWER INSTANCE MANAGEMENT
// ============================================
// Global SmilesDrawer instance for memory efficiency
let globalSmilesDrawerInstance = null;
let globalSvgDrawerInstance = null;
let smilesDrawerOptions = null;

/**
 * Get or create SvgDrawer instance for rendering
 * SmilesDrawer.parse() is a static method, not a constructor
 */
function getOrCreateSvgDrawer(options = {}) {
  if (!window.SmilesDrawer) {
    console.error('[exp_ChemistryLaTeX_Ind] SmilesDrawer not loaded!');
    return null;
  }

  // Create new instance with updated options
  const optionsKey = JSON.stringify(options);
  if (!globalSvgDrawerInstance || smilesDrawerOptions !== optionsKey) {
    globalSvgDrawerInstance = new SmilesDrawer.SvgDrawer(options);
    smilesDrawerOptions = optionsKey;
  }
  return globalSvgDrawerInstance;
}

/**
 * Queue for SMILES parsing to prevent concurrent operations crashing
 */
const parseQueue = [];
...
function queueSmilesDrawerParse(smiles, successCallback, errorCallback) {
  parseQueue.push({ smiles, successCallback, errorCallback });
  processParseQueue();
}

function processParseQueue() {
  if (activeParseCount >= MAX_CONCURRENT_PARSES || parseQueue.length === 0) {
    return;
  }

  activeParseCount++;
  const { smiles, successCallback, errorCallback } = parseQueue.shift();

  try {
    // Check if SmilesDrawer is available
    if (!window.SmilesDrawer) {
      errorCallback(new Error('SmilesDrawer not available'));
      activeParseCount--;
      processParseQueue();
      return;
    }

    // Use SmilesDrawer.parse() static method directly
...
// ============================================
// DIRECT API QUERY FUNCTIONS
// ============================================
// These functions query chemistry databases directly without a server middleware.
// Used for: PubChem (compounds), RCSB (biomolecules), COD (minerals), OPSIN (IUPAC)

/**
 * Fetch JSON via background script (bypasses CORS on strict sites like ChatGPT)
 */
async function backgroundFetchJSON(url, timeout = 15000) {
  return new Promise((resolve, reject) => {
    const timeoutId = setTimeout(() => reject(new Error('Timeout')), timeout);

    chrome.runtime.sendMessage({ type: 'FETCH_JSON', url: url }, (response) => {
      clearTimeout(timeoutId);
      if (chrome.runtime.lastError) {
        // Fallback to direct fetch if background fails
        directFetchJSON(url, timeout).then(resolve).catch(reject);
        return;
      }
...
/**
 * Direct fetch for JSON (when background not needed or fails)
 */
async function directFetchJSON(url, timeout = 15000) {
...
/**
 * Query PubChem for SMILES by compound name
 * @param {string} compoundName - Name of compound (e.g., "benzene", "aspirin")
 * @returns {object} - { canonicalSmiles, isomericSmiles, cid } or null
 */
async function queryPubChemByName(compoundName) {
  console.log('%c🧪 Querying PubChem for:', 'color: #2196F3; font-weight: bold;', compoundName);

  try {
    // First get CID
    const cidUrl = `${API_ENDPOINTS.PUBCHEM_NAME_TO_CID}${encodeURIComponent(compoundName)}/cids/JSON`;
    ...
    if (!cidData.IdentifierList || !cidData.IdentifierList.CID || cidData.IdentifierList.CID.length === 0) {
      console.warn('%c⚠️ PubChem: No CID found for', 'color: #FF9800;', compoundName);
      return null;
    }

    const cid = cidData.IdentifierList.CID[0];
    console.log('%c✅ PubChem CID:', 'color: #4CAF50;', cid);

    // Get properties including SMILES
    const propsUrl = `${API_ENDPOINTS.PUBCHEM_CID_PROPS}${cid}/property/CanonicalSMILES,IsomericSMILES,MolecularFormula,Title,IUPACName/JSON`;
    const propsData = await backgroundFetchJSON(propsUrl);

    if (!propsData.PropertyTable || !propsData.PropertyTable.Properties || propsData.PropertyTable.Properties.length === 0) {
      console.warn('%c⚠️ PubChem: No properties found for CID', 'color: #FF9800;', cid);
      return null;
    }
    ...
    return {
      cid: cid,
      canonicalSmiles: props.CanonicalSMILES?.replace(/\s+/g, ''),
      isomericSmiles: props.IsomericSMILES?.replace(/\s+/g, ''),
      formula: props.MolecularFormula,
      name: props.Title,
      iupacName: props.IUPACName
    };
  } catch (error) {
    console.warn('%c⚠️ PubChem query failed:', 'color: #FF9800;', error.message);
    return null;
  }
}

/**
 * Query PubChem autocomplete for fuzzy matches
 * @param {string} query - Partial compound name
 * @returns {array} - Array of matching compound names
 */
async function queryPubChemAutocomplete(query) {
  try {
    const url = `${API_ENDPOINTS.PUBCHEM_AUTOCOMPLETE}${encodeURIComponent(query)}/json?limit=5`;
    ...
    console.warn('%c⚠️ PubChem autocomplete failed:', 'color: #FF9800;', error.message);
    return [];
  }
}

/**
 * Query OPSIN for IUPAC name to SMILES conversion
 * @param {string} iupacName - IUPAC systematic name (e.g., "2-methylpropan-1-ol")
 * @returns {string|null} - SMILES or null
 */
async function queryOPSIN(iupacName) {
  console.log('%c🔬 Querying OPSIN for IUPAC:', 'color: #FF5722; font-weight: bold;', iupacName);

  try {
    const url = `${API_ENDPOINTS.OPSIN_IUPAC}${encodeURIComponent(iupacName)}.json`;
    const data = await backgroundFetchJSON(url);

    if (data && data.smiles) {
      console.log('%c✅ OPSIN SMILES:', 'color: #4CAF50;', data.smiles);
      return data.smiles;
    }
    return null;
  } catch (error) {
    console.warn('%c⚠️ OPSIN query failed:', 'color: #FF9800;', error.message);
    return null;
  }
}

/**
 * Query RCSB PDB for biomolecule by name
 * @param {string} name - Biomolecule name (e.g., "hemoglobin", "insulin")
 * @returns {object|null} - { pdbid, title, imageUrl } or null
 */
async function queryRCSB(name) {
  console.log('%c🧬 Querying RCSB for:', 'color: #E91E63; font-weight: bold;', name);
  ...
  try {
    ...
    const response = await fetch(API_ENDPOINTS.RCSB_SEARCH, {
      method: 'POST',
    ...
    if (!response.ok) {
      throw new Error(`RCSB search returned ${response.status}`);
    }

    const data = await response.json();

    if (!data.result_set || data.result_set.length === 0) {
      console.warn('%c⚠️ RCSB: No results for', 'color: #FF9800;', name);
      return null;
    }

    // Get the first (best) result
    const pdbid = data.result_set[0].identifier;
    console.log('%c✅ RCSB PDB ID:', 'color: #4CAF50;', pdbid);

    // Get entry details for title
    try {
      const detailsUrl = `${API_ENDPOINTS.RCSB_ENTRY}${pdbid}`;
      const details = await backgroundFetchJSON(detailsUrl);
      const title = details.struct?.title || details.rcsb_entry_info?.title || name;

      return {
        pdbid: pdbid,
        title: title,
        imageUrl: `${API_ENDPOINTS.RCSB_STRUCTURE_IMG}${pdbid.toLowerCase()}_model-1.jpeg`
      };
    } catch (e) {
      // Return with just pdbid if details fail
      return {
        pdbid: pdbid,
        title: name,
        imageUrl: `${API_ENDPOINTS.RCSB_STRUCTURE_IMG}${pdbid.toLowerCase()}_model-1.jpeg`
    ...
    console.warn('%c⚠️ RCSB query failed:', 'color: #FF9800;', error.message);
    return null;
  }
}

/**
 * Query COD for mineral by name
 * @param {string} name - Mineral name (e.g., "quartz", "calcite")
 * @returns {object|null} - { codid, name, formula, smiles } or null
 */
async function queryCOD(name) {
  console.log('%c💎 Querying COD for:', 'color: #00BCD4; font-weight: bold;', name);

  try {
    const url = `${API_ENDPOINTS.COD_SEARCH}?text=${encodeURIComponent(name)}&format=json`;
    const data = await backgroundFetchJSON(url);

    if (!data || !Array.isArray(data) || data.length === 0) {
      console.warn('%c⚠️ COD: No results for', 'color: #FF9800;', name);
      return null;
    }

    const entry = data[0];
    const codid = entry.file || entry.id;
    console.log('%c✅ COD ID:', 'color: #4CAF50;', codid);

    // Try to get SMILES from CIF file
    let smiles = null;
    try {
      const cifUrl = `${API_ENDPOINTS.COD_CIF}${codid}.cif`;
      const response = await fetch(cifUrl);
      if (response.ok) {
        const cifContent = await response.text();
        // Parse CIF for SMILES
        const smilesMatch = cifContent.match(/_chemical_smiles_canonical\s+['"]?([^'"\r\n]+)['"]?/i);
        if (smilesMatch) {
          smiles = smilesMatch[1].trim();
        }
      }
    } catch (e) {
      console.warn('%c⚠️ CIF fetch failed:', 'color: #FF9800;', e.message);
    }

    return {
      codid: codid,
      name: entry.commonname || entry.chemname || name,
      formula: entry.formula,
      smiles: smiles
    };
  } catch (error) {
    console.warn('%c⚠️ COD query failed:', 'color: #FF9800;', error.message);
    return null;
  }
}

/**
 * Try MineralNames.js fallback when COD is down
 * @param {string} name - Mineral name
 * @returns {object|null} - { codid, name, formula } or null
 */
async function queryMineralNamesFallback(name) {
  console.log('%c💎 Trying MineralNames.js fallback for:', 'color: #FF9800; font-weight: bold;', name);

  try {
    // Check if MineralNames is already loaded
    if (!window.MineralNames) {
      // Try to load it dynamically
      const scriptUrl = chrome.runtime.getURL('MineralNames.js');
    ...
    if (window.MineralNames && window.MineralNames.lookup) {
      const result = window.MineralNames.lookup(name);
      if (result) {
        console.log('%c✅ MineralNames fallback found:', 'color: #4CAF50;', result);
        return result;
      }
    }
    ...
    console.warn('%c⚠️ MineralNames fallback failed:', 'color: #FF9800;', error.message);
    return null;
  }
}
  ...
  for (const img of moleculeImages) {
    try {
      ...
      if (!isValidNomenclature(nomenclature)) {
        console.log(`[ChemRenderer] ⏭️ Skipping image - invalid/error nomenclature: "${nomenclature.substring(0, 50)}..."`);
        continue;
      }
      ...
      if (compoundType === 'compound') {
        ...
        moleculeData = {
          ...
          smiles: smiles || null,  // Optional - server uses mol= for name lookup
          type: smiles ? 'smiles' : 'nomenclature',  // Use nomenclature if no cached SMILES
          options: {
...
async function reRenderSingleMolecule(img, newSettings) {
  try {
    ...
    // Use SMILES if available, otherwise use molecule name
    const type = smiles ? 'smiles' : 'mol';
    const value = smiles || moleculeName;
...
log.info('exp_ChemistryLaTeX_Ind loaded');
...
function parseChemFlags(chemFormula, enableAIFlagControl = true) {
  ...
  const newSyntaxMatch = chemFormula.match(/chem:(smiles|mol|biomol|mineral|iupac|pbdid|cid|codid)=([^:]+):/i);
  ...
  const namedSyntaxMatch = !newSyntaxMatch ?
    chemFormula.match(/chem:([a-zA-Z0-9][a-zA-Z0-9\-,\s\(\)]{0,}?)(smiles|mol|biomol|mineral|iupac|pbdid|cid|codid)=([^:]+):/i) : null;
...
function setupLazyLoading() {
  ...
  // ============================================
  // Flow:
  // - Compounds: Query PubChem directly → Get SMILES → Render with SmilesDrawer
  // - Minerals: Query COD directly → Get SMILES/CODID → Render with SmilesDrawer or 3D viewer
  // - Biomolecules: Query RCSB directly → Get PDBID → Show 2D image or 3D viewer
  // - Direct SMILES: Render directly with SmilesDrawer
  // - IUPAC: Query OPSIN → Get SMILES → Render with SmilesDrawer
  async function renderClientSide(moleculeData, img) {
    activeLoads++;
    console.log('%c🎨 RENDER - Client-side (no server!)', 'background: #222; color: #00FF00; font-size: 16px; padding: 5px;');
    ...
    try {
      ...
      // Determine the compound type based on flags
      let compoundType = 'mol';  // Default: compound name lookup
      let lookupValue = displayName;  // Default: use display name for lookup
      let smiles = null;  // SMILES string to render

      if (flags.isDirectSmiles || moleculeData.smiles) {
        // Direct SMILES - no lookup needed
        compoundType = 'smiles';
        smiles = flags.smilesValue || moleculeData.smiles || displayName;
        lookupValue = smiles;
      ...
      console.log(`%c📍 Client-side lookup: ${compoundType}=`, 'color: #2196F3; font-weight: bold;', lookupValue);

      // ===== BIOMOLECULE: Query RCSB directly =====
      if (compoundType === 'biomol') {
        try {
          const result = await queryRCSB(lookupValue);
          if (result && result.pdbid) {
            console.log(`%c🧬 Got pdbid from RCSB: ${result.pdbid}`, 'color: #E91E63; font-weight: bold;');

            const biomoleculeData = {
              nomenclature: displayName,
              compoundType: 'biomolecule',
              embedUrl: buildMolViewEmbedUrl(result.pdbid, 'pdbid', 'biomolecule'),
              imageUrl: result.imageUrl || buildBioImageUrl(result.pdbid),
              pdbid: result.pdbid,
      ...
      // ===== MINERAL: Query COD directly =====
      if (compoundType === 'mineral') {
        try {
          // Try COD first, fallback to MineralNames.js
          let result = await queryCOD(lookupValue);
          if (!result) {
            result = await queryMineralNamesFallback(lookupValue);
          }

          if (result && result.codid) {
            console.log(`%c💎 Got CODID: ${result.codid}`, 'color: #00BCD4; font-weight: bold;');

            // If mineral has SMILES, render 2D with SmilesDrawer
            if (result.smiles && !flags.is3D) {
              smiles = result.smiles;
              moleculeData.codid = result.codid;
              // Fall through to SmilesDrawer rendering below
            } else {
              // Show 3D viewer for minerals (default if no SMILES)
              const mineralData = {
                nomenclature: displayName,
                compoundType: 'mineral',
                embedUrl: `https://embed.molview.org/v1/?codid=${result.codid}`,
                codid: result.codid,
            ...
            console.warn('%c⚠️ Mineral not found:', 'color: #FF9800;', lookupValue);
            img.alt = `Mineral not found: ${lookupValue}`;
            decrementActiveLoads();
            return;
          }
        } catch (e) {
          console.error('Mineral lookup failed:', e.message);
          img.alt = `Mineral lookup failed: ${lookupValue}`;
          decrementActiveLoads();
          return;
        }
      }

      // ===== IUPAC: Query OPSIN directly =====
      if (compoundType === 'iupac' && !smiles) {
        try {
          smiles = await queryOPSIN(lookupValue);
          if (!smiles) {
            console.warn('%c⚠️ OPSIN could not parse IUPAC name:', 'color: #FF9800;', lookupValue);
            // Fallback to PubChem for IUPAC names
            const pubchemResult = await queryPubChemByName(lookupValue);
            if (pubchemResult) {
              smiles = settings.m2cfUse3DSmiles
                ? (pubchemResult.isomericSmiles || pubchemResult.canonicalSmiles)
                : (pubchemResult.canonicalSmiles || pubchemResult.isomericSmiles);
              moleculeData.cid = pubchemResult.cid;
            }
          }
        } catch (e) {
          console.error('IUPAC lookup failed:', e.message);
        }
      }

      // ===== COMPOUND: Query PubChem directly =====
      if (compoundType === 'mol' && !smiles) {
        try {
          const result = await queryPubChemByName(lookupValue);
          if (result) {
            smiles = settings.m2cfUse3DSmiles
              ? (result.isomericSmiles || result.canonicalSmiles)
              : (result.canonicalSmiles || result.isomericSmiles);
            moleculeData.cid = result.cid;
            console.log(`%c🧪 Got SMILES from PubChem: ${smiles?.substring(0, 30)}...`, 'color: #4CAF50; font-weight: bold;');
          } else {
            // Try autocomplete for fuzzy matches
            const matches = await queryPubChemAutocomplete(lookupValue);
            if (matches.length > 0) {
              const bestMatch = matches[0];
              const retryResult = await queryPubChemByName(bestMatch);
              if (retryResult) {
                smiles = settings.m2cfUse3DSmiles
                  ? (retryResult.isomericSmiles || retryResult.canonicalSmiles)
                  : (retryResult.canonicalSmiles || retryResult.isomericSmiles);
                moleculeData.cid = retryResult.cid;
                console.log(`%c🧪 Got SMILES via autocomplete match "${bestMatch}": ${smiles?.substring(0, 30)}...`, 'color: #4CAF50;');
              }
            }
          }
        } catch (e) {
          console.error('PubChem lookup failed:', e.message);
        }
      }
      ...
      // ===== SMILES NOT FOUND =====
      if (!smiles) {
        console.error('%c❌ Could not find SMILES for:', 'color: red;', lookupValue);
        img.alt = `Could not find SMILES for: ${lookupValue}`;
        decrementActiveLoads();
        return;
      }

      // ===== 2D RENDERING WITH SMILESDRAWER =====
      console.log('%c🎨 Rendering with SmilesDrawer:', 'background: #4CAF50; color: white; font-weight: bold; padding: 4px;', smiles.substring(0, 50));
      ...
      const theme = settings.sdTheme || 'light';
      ...
      // Intelligent size scaling based on molecule complexity
      if (settings.sdScaleByWeight && smiles) {
        ...
        const sizeMultiplier = Math.max(0.7, Math.min(4, Math.sqrt(heavyAtomCount) / Math.sqrt(20)));
        renderWidth = Math.round(baseWidth * sizeMultiplier);
        renderHeight = Math.round(baseHeight * sizeMultiplier);
        console.log(`📐 Intelligent sizing: ${heavyAtomCount} atoms → ${sizeMultiplier.toFixed(2)}x scale`);
      }
      ...
      const smilesDrawerOptions = {
        ...
        terminalCarbons: showMethyl,
        explicitHydrogens: addHydrogens,
        showHydrogens: addHydrogens,
        showImplicitHydrogens: showImplicitH,
        showAromaticRings: aromaticCircles,
        atomNumbering: showAtomNumbers,
        solidBondColors: !useGradientColors,
        ...
        themes: {
          dark: {
            ...
            ATOM_NUMBER: '#ffd700', BACKGROUND: 'transparent'
          },
          light: {
            C: '#222222', O: '#e74c3c', N: '#3498db', F: '#27ae60',
            CL: '#16a085', BR: '#d35400', I: '#8e44ad', P: '#d35400',
            S: '#f1c40f', B: '#e67e22', SI: '#e67e22', H: '#666666',
            ATOM_NUMBER: '#2196F3', BACKGROUND: 'transparent'
          }
        }
      };

      // Render with SmilesDrawer using the queue system
      queueSmilesDrawerParse(smiles, (tree) => {
        let tempDiv = null;
        ...
        try {
          ...
          // Draw the molecule
          const drawer = getOrCreateSvgDrawer(smilesDrawerOptions);
          drawer.draw(tree, tempId, theme);
          ...
          if (renderedSvg) {
            ...
            try {
              ...
              const padding = 10;
              renderedSvg.setAttribute('width', Math.ceil(bbox.width + padding * 2));
              renderedSvg.setAttribute('height', Math.ceil(bbox.height + padding * 2));
            ...
            svgImg.dataset.compoundType = compoundType;

            svgImg.style.cssText = 'display: block; margin: 0; padding: 0; border: none;';
            ...
            if (img.parentNode) {
              ...
              console.log('%c✅ SmilesDrawer rendered successfully', 'color: green; font-weight: bold;');
            }
          }
        } catch (renderError) {
          console.error('%c❌ SmilesDrawer render error:', 'color: red;', renderError);
        } finally {
          // Clean up temp div
        ...
        console.error('%c❌ SmilesDrawer parse error:', 'color: red;', parseError);
        img.alt = `Error parsing SMILES: ${parseError.message}`;
      });
  ...
  const newObserver = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      ...
      if (entry.isIntersecting && img.dataset.loaded !== 'true' && img.dataset.loaded !== 'loading') {
        // Check if this is a chemistry image (MoleculeViewer, compound, or Mol2chemfig)
        if (img.classList.contains('molecule-viewer') || img.classList.contains('molecule-compound') || img.classList.contains('molecule-legacy')) {
...
function wrapChemicalFormulas(text) {
  ...
  const chemPatternRegex = /\bchem:([^=]*?)(smiles|mol|biomol|mineral|iupac|pbdid|cid|codid)=([^:]+):/gi;
  ...
  if (chemMatches) {
    ...
    result = result.replace(/\bchem:([^=]*?)(smiles|mol|biomol|mineral|iupac|pbdid|cid|codid)=([^:]+):/gi, (match, displayName, type, valueWithFlags) => {
      try {
