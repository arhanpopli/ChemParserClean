...
(function () {
    ...
    function startMatterAnimation() {
        ...
        // Use explicit check so 0 is a valid count
        var count = (typeof targetMoleculeCount === 'number') ? targetMoleculeCount : 20;
        ...
        function rebuildScene() {
            ...
            // Use explicit undefined check so that 0 is a valid count
            var effectiveCount = (typeof targetMoleculeCount === 'number') ? targetMoleculeCount : 20;
            count = Math.max(0, Math.min(effectiveCount, keys.length));
            ...
            try {
                if (window.chrome && chrome.storage && chrome.storage.sync) {
                    chrome.storage.sync.get({
                        moleculeCount: (typeof targetMoleculeCount === 'number') ? targetMoleculeCount : 20,
        ...
        window.updateMoleculeAnimation = function () {
            try {
                if (window.chrome && chrome.storage && chrome.storage.sync) {
                    var defaultCount = (typeof targetMoleculeCount === 'number') ? targetMoleculeCount : 20;
                    chrome.storage.sync.get({ moleculeCount: defaultCount, moleculeScale: globalScale || 0.4, cursorGravityStrength: -0.9, initialVelocity: initialVelocity }, function (s) {
                        if (typeof s.moleculeCount === 'number') targetMoleculeCount = s.moleculeCount;
                        if (typeof s.moleculeScale === 'number') globalScale = s.moleculeScale;
                        if (typeof s.cursorGravityStrength === 'number') cursorGravityStrength = s.cursorGravityStrength;
                        if (typeof s.initialVelocity === 'number') initialVelocity = s.initialVelocity;
