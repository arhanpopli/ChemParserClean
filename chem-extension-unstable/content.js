...
let settings = {
  ...
  // SmilesDrawer rendering options (sd* prefix - used by popup.js)
  sdShowCarbons: false,
  ...
  sdTheme: 'light',
  // mol2chemfig rendering options (legacy, kept for compatibility)
...
chrome.storage.sync.get(null, (result) => {
  ...
  // Debug: Log SmilesDrawer specific settings
  console.log('%c[SmilesDrawer Settings from Storage]', 'background: #E91E63; color: white; padding: 2px 6px;', {
...
function applyFlagOverrides(baseSettings, flagOverrides) {
  try {
    ...
    // Apply overrides based on flags - update BOTH sd* and m2cf* settings for full compatibility
    if (flagOverrides?.showCarbons !== undefined) {
      newSettings.sdShowCarbons = flagOverrides.showCarbons;
      newSettings.m2cfShowCarbons = flagOverrides.showCarbons;
    }
    if (flagOverrides?.aromaticCircles !== undefined) {
      newSettings.sdAromaticRings = flagOverrides.aromaticCircles;
      newSettings.m2cfAromaticCircles = flagOverrides.aromaticCircles;
    }
    if (flagOverrides?.showMethyls !== undefined) {
      newSettings.sdShowMethyls = flagOverrides.showMethyls;
      newSettings.m2cfShowMethyls = flagOverrides.showMethyls;
    }
    if (flagOverrides?.atomNumbers !== undefined) {
      newSettings.sdAtomNumbers = flagOverrides.atomNumbers;
      newSettings.m2cfAtomNumbers = flagOverrides.atomNumbers;
    }
    if (flagOverrides?.addHydrogens !== undefined) {
      newSettings.sdShowHydrogens = flagOverrides.addHydrogens;
      newSettings.m2cfAddH2 = flagOverrides.addHydrogens;
    }
    if (flagOverrides?.flipHorizontal !== undefined) {
      newSettings.sdFlipHorizontal = flagOverrides.flipHorizontal;
    ...
    if (flagOverrides?.flipVertical !== undefined) {
      newSettings.sdFlipVertical = flagOverrides.flipVertical;
...
function setupLazyLoading() {
  ...
  async function renderClientSide(moleculeData, img) {
    ...
    try {
      ...
      // Get SmilesDrawer rendering options from settings
      // Support both sd* (SmilesDrawer) and m2cf* (mol2chemfig) setting names for compatibility
      // Then apply any per-molecule flag overrides (e.g., +c, +m, /d-c)
      let showCarbons = settings.sdShowCarbons === true || settings.m2cfShowCarbons === true;
      let aromaticCircles = settings.sdAromaticRings === true || settings.m2cfAromaticCircles === true;
      let showMethyl = settings.sdShowMethyls === true || settings.m2cfShowMethyls === true;
      let showAtomNumbers = settings.sdAtomNumbers === true || settings.m2cfAtomNumbers === true;
      let flipVertical = settings.sdFlipVertical === true || settings.m2cfFlipVertical === true;
      let flipHorizontal = settings.sdFlipHorizontal === true || settings.m2cfFlipHorizontal === true;
      let addHydrogens = settings.sdShowHydrogens === true || settings.m2cfAddH2 === true;
      let customSize = null;
      let customRotation = null;

      // DEBUG: Log raw settings values and computed values
      console.log('%c[DEBUG] Raw settings values:', 'background: #FF5722; color: white; padding: 2px 6px;', {
        'settings.sdShowCarbons': settings.sdShowCarbons,
        'typeof settings.sdShowCarbons': typeof settings.sdShowCarbons,
        'settings.sdShowCarbons === true': settings.sdShowCarbons === true
      });
      console.log('%c[DEBUG] Computed values BEFORE flags:', 'background: #2196F3; color: white; padding: 2px 6px;', {
        showCarbons, aromaticCircles, showMethyl, showAtomNumbers, addHydrogens
      });

      // Apply per-molecule flag overrides if present
      const flags = moleculeData.flags || moleculeData.options || {};
      console.log('%c[DEBUG] flags object:', 'background: #9C27B0; color: white;', flags);
      if (flags) {
      ...
      console.log('%c[DEBUG] Computed values AFTER flags:', 'background: #4CAF50; color: white; padding: 2px 6px;', {
        showCarbons, aromaticCircles, showMethyl, showAtomNumbers, addHydrogens
      });
      ...
      console.log('%c[SmilesDrawer Settings]', 'background: #9C27B0; color: white; padding: 2px 6px;', {
        'settings.sdShowCarbons': settings.sdShowCarbons,
        'settings.sdShowHydrogens': settings.sdShowHydrogens,
        'settings.sdAtomNumbers': settings.sdAtomNumbers,
        'settings.sdShowMethyls': settings.sdShowMethyls,
        'settings.sdAromaticRings': settings.sdAromaticRings,
        'computed showCarbons': showCarbons,
        'computed addHydrogens': addHydrogens,
        'computed showAtomNumbers': showAtomNumbers,
        'computed showMethyl': showMethyl,
        'computed aromaticCircles': aromaticCircles
      });
      ...
      const smilesDrawerOptions = {
        ...
        showCarbons: showCarbons,              // Show all carbon atoms (C labels)
        showHydrogens: addHydrogens,           // Show hydrogen atoms
        atomNumbering: showAtomNumbers,        // Show atom numbers
        terminalCarbons: showMethyl || showCarbons,  // Show terminal carbons (CH3)
        explicitHydrogens: addHydrogens,
        compactDrawing: !showCarbons,          // Compact mode hides carbons
        showAromaticRings: aromaticCircles,    // Show aromatic rings as circles
      ...
      // Check if this is an ionic/fragmented SMILES (contains '.' meaning disconnected fragments)
      // SvgDrawer renders these as stacked atoms, but Canvas Drawer renders them correctly
      const isIonicSmiles = smiles.includes('.');
      ...
      SmilesDrawer.parse(smiles, function (tree) {
        ...
        // ===== IONIC SMILES: Use Canvas Drawer with special options =====
        if (isIonicSmiles) {
          // Special options for ionic SMILES - matches mineral_test.html but respects user settings
          const ionicOptions = {
            width: adjustedWidth,
            ...
            shortBondLength: 0.8,
            bondSpacing: 4,
            atomVisualization: showAtomNumbers ? 'allballs' : 'default',
            isomeric: true,
            debug: false,
            // User-controllable options (same as regular SMILES)
            showCarbons: showCarbons,              // Show all carbon atoms (C labels)
            showHydrogens: addHydrogens,           // Show hydrogen atoms
            atomNumbering: showAtomNumbers,        // Show atom numbers
            terminalCarbons: showMethyl || showCarbons || true,  // Default true for ionic, but respect user settings
            explicitHydrogens: addHydrogens,  // Respect user's hydrogen setting
            overlapSensitivity: 0.42,
            overlapResolutionIterations: 1,
            compactDrawing: false,  // Must be FALSE for ionic to render properly
            fontSizeLarge: 11,
            fontSizeSmall: 6,
            padding: 20,
            themes: {
              dark: {
                C: '#ffffff', O: '#ff6b6b', N: '#4dabf7', S: '#fcc419',
                P: '#fd7e14', F: '#51cf66', Cl: '#20c997', Br: '#fd7e14',
                I: '#be4bdb', Si: '#adb5bd', Ca: '#20c997', Mg: '#12b886',
                Fe: '#fd7e14', Na: '#845ef7', K: '#845ef7',
                BACKGROUND: '#1a1a2e'
              },
              light: {
                C: '#222', O: '#e74c3c', N: '#3498db', S: '#f1c40f',
                P: '#e67e22', F: '#27ae60', Cl: '#27ae60', Br: '#c0392b',
                I: '#8e44ad', Si: '#95a5a6', Ca: '#1abc9c', Mg: '#16a085',
                Fe: '#d35400', Na: '#9b59b6', K: '#9b59b6',
                BACKGROUND: '#ffffff'
              }
            }
          };

          const tempCanvasId = `temp-canvas-${Date.now()}`;
          const tempCanvas = document.createElement('canvas');
          tempCanvas.id = tempCanvasId;
          // Set dimensions as attributes (required for canvas)
          tempCanvas.width = adjustedWidth;
          tempCanvas.height = adjustedHeight;
          // Make canvas visible but off-screen (required for SmilesDrawer to render)
          tempCanvas.style.cssText = 'position:absolute;left:-9999px;top:-9999px;visibility:visible;';
          document.body.appendChild(tempCanvas);

          // Use Canvas Drawer with ionic-specific options
          const canvasDrawer = new SmilesDrawer.Drawer(ionicOptions);
          canvasDrawer.draw(tree, tempCanvasId, theme, false);

          // Use setTimeout to ensure canvas has finished rendering
          setTimeout(() => {
            const canvasEl = document.getElementById(tempCanvasId);
            if (canvasEl) {
              try {
          ...
          return; // Exit - ionic rendering complete
        }

        // ===== REGULAR SMILES: Use SVG Drawer =====
  ...
  async function querySearchAPI(moleculeData) {
    try {
      ...
      // console.log('%c✅ Search API response:', 'color: #4CAF50; font-weight: bold;', data);
      ...
      console.error('%c❌ Search API error:', 'color: red; font-weight: bold;', error);
  ...
  async function loadMoleculeViewerImage(img) {
    activeLoads++;
    // console.log('%c🧪 [loadMoleculeViewerImage] CALLED', 'background: #4CAF50; color: white; font-weight: bold; padding: 4px;', img);
    // console.log('%c📸 [loadMoleculeViewerImage] Image element:', 'color: #4CAF50;', img);
    // console.log('%c📋 [loadMoleculeViewerImage] Dataset:', 'color: #4CAF50;', img.dataset);
    ...
    try {
      ...
      // console.log('%c📦 [loadMoleculeViewerImage] Decoded molecule data:', 'color: #4CAF50; font-weight: bold;', moleculeData);

      // ========================================
      // STEP 1: Query Search API (port 8001) - ALWAYS QUERY FIRST FOR ALL RENDERERS
      // ========================================
      // console.log('%c🔍 [loadMoleculeViewerImage] Calling querySearchAPI...', 'color: #FF9800; font-weight: bold;');
      const searchData = await querySearchAPI(moleculeData);
      // console.log('%c✅ [loadMoleculeViewerImage] querySearchAPI returned:', 'color: #4CAF50; font-weight: bold;', searchData);
  ...
  async function querySearchAPI(moleculeData) {
    // console.log('%c🔍 [querySearchAPI] CALLED', 'background: #2196F3; color: white; font-weight: bold; padding: 4px;');
    // console.log('%c📦 [querySearchAPI] moleculeData:', 'color: #2196F3; font-weight: bold;', moleculeData);

    // Determine the query string (nomenclature or SMILES)
    let queryString = '';
    if (moleculeData.nomenclature) {
      queryString = moleculeData.nomenclature;
      // console.log('%c✅ [querySearchAPI] Using nomenclature:', 'color: green;', queryString);
    } else if (moleculeData.smiles) {
      queryString = moleculeData.smiles;
      // console.log('%c✅ [querySearchAPI] Using SMILES:', 'color: green;', queryString);
      ...
      // console.log('%c✅ [querySearchAPI] Using fallback SMILES:', 'color: green;', queryString);
    } else {
      console.error('%c❌ [querySearchAPI] No valid identifier found!', 'color: red; font-weight: bold;');
    ...
    // console.log('%c🌐 [querySearchAPI] Calling search API:', 'color: #2196F3;', searchUrl);

    const searchResult = await backgroundFetchJSON(searchUrl);
    // console.log('%c📊 [querySearchAPI] Search result:', 'color: #2196F3; font-weight: bold;', searchResult);
  ...
  function loadMoleculeImage(img) {
    // console.log('%c🔍 loadMoleculeImage CALLED', 'background: #9C27B0; color: white; padding: 3px 6px;', img);
    ...
    result = result.replace(/\bchem:([^:]+):/g, (match, content) => {
      ...
      if (content_trimmed.includes('/') || content_trimmed.includes('+') || content_trimmed.includes('-')) {
        try {
          ...
          // - If /d flag is present: use global defaults, then apply +/- flags to modify
          // - If no /d flag: start with everything OFF, only enable what's explicitly +
          let baseSettings;
          if (flagOverrides.useDefaults) {
            // /d flag present: START WITH USER'S DEFAULTS, then apply +/- overrides
            // Example: chem:histamine/d+c-o: → use defaults + add carbons - remove aromatic circles
            baseSettings = { ...settings };
          } else {
            // No /d flag: START WITH EVERYTHING OFF, only apply + flags
            // Example: chem:histamine/+c+h: → only carbons and hydrogens, everything else OFF
            // The - flags are meaningless here since everything starts OFF
            baseSettings = {
              ...settings,
              // Clear all display options (both sd* and m2cf* for compatibility)
              sdShowCarbons: false,
              sdAromaticRings: false,
              sdShowMethyls: false,
              sdAtomNumbers: false,
              sdShowHydrogens: false,
              sdFlipHorizontal: false,
      ...
      const moleculeViewerData = {
        ...
        options: {
          ...
          // Use sd* (SmilesDrawer) settings with m2cf* fallback for compatibility
          aromaticCircles: currentSettings.sdAromaticRings || currentSettings.m2cfAromaticCircles,
          fancyBonds: currentSettings.m2cfFancyBonds,
          showCarbons: currentSettings.sdShowCarbons || currentSettings.m2cfShowCarbons,
          showMethyls: currentSettings.sdShowMethyls || currentSettings.m2cfShowMethyls,
          atomNumbers: currentSettings.sdAtomNumbers || currentSettings.m2cfAtomNumbers,
          hydrogensMode: currentSettings.m2cfHydrogensMode,
          addH2: currentSettings.sdShowHydrogens || currentSettings.m2cfAddH2,
          flipHorizontal: currentSettings.sdFlipHorizontal || currentSettings.m2cfFlipHorizontal,
          flipVertical: currentSettings.sdFlipVertical || currentSettings.m2cfFlipVertical,
